...
## Initial SURF Data Cleaning, Joining
...
```{r}
# SURF Sed
SURF.Sed.sf <- st_join(SURF.Sed.sf, USFE.RiskRegions[1], left = T) %>% # left join of risk region numbers to SFEI data
  filter(!is.na(Subregion)) %>%
  filter(between(Sample_date, as_date("2009-10-01"), as_date("2019-09-30")))

#SURF Water
SURF.WQ.sf <- st_join(SURF.WQ.sf, USFE.RiskRegions[1], left = T) %>% # left join of risk region numbers to SFEI data
  filter(!is.na(Subregion)) %>%
  filter(between(Sample_date, as_date("2009-10-01"), as_date("2019-09-30")))
...
## Check SURF and CEDEN for duplicates
