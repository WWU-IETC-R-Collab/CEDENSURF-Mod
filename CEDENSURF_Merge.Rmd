...
Date range of SURF water data: from `r min(as.Date(SURFMod_WQ$Sample_date))` to `r max(as.Date(SURFMod_WQ$Sample_date))`

Date range of SURF sediment data: from from `r min(as.Date(SURFMod_SED$Sample_date))` to `r max(as.Date(SURFMod_SED$Sample_date))`
...
YES THEY ARE. In the original CEDEN data download, the results column is empty for these records. The comments suggest these data are also recorded elsewhere, so perhaps it was a mistake to include no result here. 
